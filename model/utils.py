...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        video_name = '\\'.join(self.samples[index].split('/')[-1].split('.')[-2].split('\\')[:2]) # local/colab
        #frame_name = int(self.samples[index].split('/')[-1].split('.')[-2])
        frame_name = int(self.samples[index].split('/')[-1].split('.')[0].split('\\')[-1]) # local
        # frame_name = int(self.samples[index].split('/')[-1].split('.')[0][-3:]) # colab
